...
class EKYCPassport extends StatefulWidget {
  const EKYCPassport({Key? key}) : super(key: key);

  @override
  State<EKYCPassport> createState() => _EKYCPassportState();
}

class _EKYCPassportState extends State<EKYCPassport> {
  @override
  void initState() {
    super.initState();
    ...
    return BlocBuilder<EkycBloc, EkycState>(
      builder: (context, state) {
        if (state is EkycCameraReadyState) {
          return Scaffold(
            appBar: MyAppBar('Xác thực Hộ chiếu'),
            body: Stack(
                    ...
                    children: [
                      Container(
                        ...
                        color: state.currentStep == 2
                            ...
                            : ColorsPrimary.Lv1,
                        child: Text(
                          'Vui lòng đặt Hộ chiếu nằm trong vùng chọn. Ảnh chụp trong điều kiện đủ sáng, rõ nét.',
      ...
      child: Column(
        children: state.currentStep == 2
                      ...
                      child: ButtonPrimary(
                        onTap: () {
                          AppBlocs.ekycBloc.add(EkycUploadFileEvent(
                            file: File(
                                AppBlocs.ekycBloc.images['passport']?.path ??
                                    ''),
                            fileName: 'passport',
            ...
            : [
                ...
                SizedBox(height: 8.px),
                Text(
                  'Chụp ảnh của Hộ chiếu',
                  style: AppTextStyle.textStyle.s16().w500().cN5(),
                ),
                SizedBox(height: 20.px),
                GestureDetector(
                  onTap: () {
                    AppBlocs.ekycBloc.add(EkycTakePassportPictureEvent());
                  },
                  child: Container(
                    width: 66.px,
                    height: 66.px,
...
class PassportOverlayShape extends ShapeBorder {
  PassportOverlayShape({
  ...
  })  : cutOutWidth = cutOutWidth ?? cutOutSize ?? 256.px,
        cutOutHeight = cutOutHeight ?? cutOutSize ?? 347.px {
    assert(
      borderLength <=
